...
{
	public class DotnetGlobalToolTests : TemplateTests
	{
		public DotnetGlobalToolTests(ITestOutputHelper outputHelper, GlobalSetupFixture data) : base(outputHelper, data)
		{
		}
		...
		public async Task MandatoryParameters()
		{
			...
			{
				var ex = await Assert.ThrowsAsync<ScaffoldingFailedException>(async() => await Dotnet.Cli.NewAsync("global-tool", "", CancellationToken.None));
